...
def fft_identity(framelength, overlap, padding):
    ...
    original = numpy.hstack(
        (
            numpy.zeros((padlength // 2,)),
            original,
            numpy.zeros((padlength // 2,))
        )
    )
    ...
    for i in values:
        spectrum = Transform.stft(
            original[i:i + framelength], padding=padding
        ) / (overlap // 2)
    ...
    assert numpy.allclose(
        original[padlength // 2:-padlength // 2],
        output[padlength // 2:-padlength // 2],
